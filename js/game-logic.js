...
async function checkGuess() {
  try {
    ...
    const opponentCharacter = isRoomOwner
      ? roomData.sortedCharacterVisitor
      : roomData.sortedCharacterOwner;
    const opponentCharacterName = opponentCharacter
      .split("/")
      .pop()
      .replace(".jpg", "");
    ...
    if (guessedCharacter === opponentCharacterName) {
      ...
      if (wrongAttempts >= MAX_WRONG_ATTEMPTS) {
        ...
        alert(
          `❌ Errou! Seu oponente foi notificado. Tentativas: ${wrongAttempts}/${MAX_WRONG_ATTEMPTS}`
        );
      }
    }
  } catch (error) {
    ...
    alert(
      `Erro: ${error.message || "Ocorreu um erro ao verificar seu palpite"}`
    );
  }
}
  ...
  roomListener = database.ref(`rooms/${roomId}`).on("value", (snapshot) => {
    ...
    if (roomData.roomId) {
      document.getElementById(
        "room-id"
      ).textContent = `ID da Sala: ${roomData.roomId}`;
    }
    ...
    if (
      ...
      const diceValue = isRoomOwner
        ? roomData.diceValues.owner
        : roomData.diceValues.visitor;
      const opponentDiceValue = isRoomOwner
        ? roomData.diceValues.visitor
        : roomData.diceValues.owner;
        ...
        database
          ...
          .then((userSnap) => {
            const currentUserName =
              userSnap.val().username || currentUser.email.split("@")[0];

            database
              .ref(`users/${opponentUid}`)
              .once("value")
              .then((opponentSnap) => {
                const opponentName =
                  opponentSnap.val().username ||
                  opponentSnap.val().email.split("@")[0];
                const quemComeca =
                  diceValue > opponentDiceValue
                    ? currentUserName
                    : opponentName;
                const startingPlayer =
                  diceValue > opponentDiceValue ? currentUser.uid : opponentUid;
...
function sortearPersonagem() {
  ...
  database
    .ref(`rooms/${roomId}/charactersSorted`)
    .once("value")
    .then((snapshot) => {
      ...
      setTimeout(() => {
        ...
        database
          ...
          .then((roomSnapshot) => {
            ...
            if (
              roomData.ownerCharacterReady &&
              roomData.visitorCharacterReady
            ) {
              database.ref(`rooms/${roomId}`).update({
...
function sortearDado() {
  ...
  database
    .ref(`rooms/${roomId}/diceResultsShown`)
    .once("value")
    .then((snapshot) => {
      if (snapshot.exists() && snapshot.val()) {
