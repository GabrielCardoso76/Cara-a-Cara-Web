window.getRanking = async function (limit = 50) {
  // Agora aceita um parâmetro de limite
  try {
    ...
    snapshot.forEach((childSnapshot) => {
      ...
      users.push({
        ...
        username:
          ...
          (userData.email ? userData.email.split("@")[0] : "Jogador"),
        wins: wins,
        losses: losses,
        matches: matches,
        winRate: winRate,
...
window.displayRanking = function (users) {
  ...
  if (!users || users.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6">Nenhum dado de ranking disponível ainda.</td></tr>';
    return;
  }
