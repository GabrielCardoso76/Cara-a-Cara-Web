...
function loadInitialMessages(messages) {
    Object.keys(messages).forEach(messageId => {
        if (!chatMessages[messageId]) {
            ...
            const name = msg.senderName || (msg.senderUid ? 
                '<span class="loading-name">Carregando...</span>' : 'Anônimo');
            ...
            if (name === 'Carregando...' && msg.senderUid) {
                getDisplayName(msg.senderUid).then(name => {
                    ...
                    database.ref(`rooms/${roomId}/messages/${messageId}/senderName`).set(name);
                });
            }
        }
    });
