...
function cleanupListeners() {
  ...
  if (notificationsListener) {
    database
      .ref(`rooms/${roomId}/notifications`)
      .off("child_added", notificationsListener);
...
function setupNotificationListener() {
  ...
  notificationsListener = database
    .ref(`rooms/${roomId}/notifications`)
    .on("child_added", (snapshot) => {
      ...
      if (notification.from !== currentUser.uid) {
        switch (notification.type) {
          case "wrong_guess":
            showCustomAlert(
              `🚨 ${notification.fromName} tentou adivinhar "${notification.guessedCharacter}" e errou! (${notification.attempts})`
            );
            break;
        }
